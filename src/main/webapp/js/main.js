...
let loanTenureYears = parseFloat(loanTenureInput.value);
...
const calculateEMI = () => {
  checkValues();
  refreshInputValues();
  let emi = loanAmount * interest * (Math.pow(1 + interest, loanTenureMonths) / (Math.pow(1 + interest, loanTenureMonths) - 1));
  return emi;
};
